...
# Change appropriate variable for testing
...
# Initializing parameter for penalty computation 
C = 3 # constraint is for role pg before change
q = 5 
N = 7
y = q - C
z = N - q 
# Scaling constants for Minor vs Major Penalty 
# m is constant for minor penalty 
# M is constant for Major penalty  
# increase 150 every tier (subject to change)

#scale with penalty 

salary_tier_threshold = 100000000

if salary_cap <  salary_tier_threshold:  # first tier of penalty, tier salary threshold 
    
    # scaling m and M according to total budget
    m = salary_cap * 0.000003
    M = salary_cap * 0.000006 
elif salary_cap < (salary_tier_threshold * 2): # second tier (2 * first tier salary threshold)
    
    # twice the m and M from first tier as salary threshold is doubled as well
    m = salary_cap * 0.000006
    M = salary_cap * 0.000012
elif salary_cap < (salary_tier_threshold * 3): # third tier (3 * first tier salary threshold)
    
    # 3 times the m and M from first tier as salary threshold is tripled
    m = salary_cap * 0.000009
    M = salary_cap * 0.000018
elif salary_cap >= (salary_tier_threshold * 3): 
    m = salary_cap * 0.000012
    M = salary_cap * 0.000024

# Old penalty system
#if salary_cap <  50000000:
#    m = 100
#    M = 500
#elif salary_cap < 70000000:
#    m = 250
#    M = 650
#elif salary_cap < 90000000:
#    m = 400
#    M = 800
#elif salary_cap < 110000000:
#    m = 550
#    M = 950
#elif salary_cap < 130000000:
#    m = 700
#    M = 1100
#else:
#    m = 850
